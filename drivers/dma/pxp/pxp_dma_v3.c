...
static void pxp_set_oln(int layer_no, struct pxps *pxp)
{
	...
	u32 pitch = olparams_data->stride ? olparams_data->stride : olparams_data->width;
...
static void pxp_set_olparam(int layer_no, struct pxps *pxp)
{
	...
	else if (olparams_data->pixel_fmt == PXP_PIX_FMT_BGRA32)
	{
		...
		{
			olparam |=
				BF_PXP_AS_CTRL_ALPHA_CTRL(BV_PXP_AS_CTRL_ALPHA_CTRL__ROPs);
			olparam |= 0x3 << 16;
		}
	}
	else if (olparams_data->pixel_fmt == PXP_PIX_FMT_RGB565)
	...
	if (olparams_data->global_alpha_enable)
	{
		...
		{
			olparam |=
				BF_PXP_AS_CTRL_ALPHA_CTRL(BV_PXP_AS_CTRL_ALPHA_CTRL__Override);
		}
		else
		{
			olparam |=
				BF_PXP_AS_CTRL_ALPHA_CTRL(BV_PXP_AS_CTRL_ALPHA_CTRL__Multiply);
		}
...
static void pxp_set_lut(struct pxps *pxp)
{
	...
	else if (((lut_op & PXP_LUT_INVERT) != 0) && ((lut_op & PXP_LUT_BLACK_WHITE) != 0))
	{
		/* Fill out LUT table with inverted monochromized values */

		/* clear bypass bit, set lookup mode & out mode */
		__raw_writel(BF_PXP_LUT_CTRL_LOOKUP_MODE(BV_PXP_LUT_CTRL_LOOKUP_MODE__DIRECT_Y8) |
						 BF_PXP_LUT_CTRL_OUT_MODE(BV_PXP_LUT_CTRL_OUT_MODE__Y8),
					 pxp->base + HW_PXP_LUT_CTRL);
...
static void pxp_set_s0buf(struct pxps *pxp)
{
	...
	dma_addr_t Y1, U1, V1;
	u32 offset, bpp = 1;
	u32 pitch = s0_params->stride ? s0_params->stride : s0_params->width;
...
static void pxp_wfe_b_process(struct pxps *pxp)
{
	...
	__raw_writel(
		BF_PXP_WFE_B_STORE_ADDR_0_CH0_OUT_BASE_ADDR0(store_ch0->paddr + (store_ch0->left + store_ch0->top * store_ch0->stride) * 2),
		pxp->base + HW_PXP_WFE_B_STORE_ADDR_0_CH0);
...
static void pxp_histogram_status_report(struct pxps *pxp, u32 *hist_status, u32 *pixel_nums)
{
	...
	*hist_status = (__raw_readl(pxp->base + HW_PXP_HIST_B_CTRL) & BM_PXP_HIST_B_CTRL_STATUS) >> BP_PXP_HIST_B_CTRL_STATUS;
...
static const struct of_device_id imx_pxpdma_dt_ids[] = {
	{
		...
		.data = &imx_pxpdma_devtype[0],
	},
	{
		.compatible = "fsl,imx6ull-pxp-dma",
		.data = &imx_pxpdma_devtype[1],
	},
	{/* sentinel */}};
